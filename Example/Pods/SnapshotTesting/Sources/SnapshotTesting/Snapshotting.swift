...
public struct Snapshotting<Value, Format> {
    ...
    public func pullback<NewValue>(_ transform: @escaping (_ otherValue: NewValue) -> Value) -> Snapshotting<NewValue, Format> {
        asyncPullback { newValue in Async(value: transform(newValue)) }
    }
    ...
    {
        Snapshotting<NewValue, Format>(pathExtension: pathExtension,
                                       diffing: diffing) { newValue in
            .init { callback in
...
public extension Snapshotting where Value == Format {
    init(pathExtension: String?, diffing: Diffing<Format>) {
        self.init(pathExtension: pathExtension,
                  diffing: diffing,
                  snapshot: { $0 })
    }
}
