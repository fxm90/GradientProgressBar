    ...
    public extension Snapshotting where Value == UIViewController, Format == UIImage {
        ...
        static func image(on config: ViewImageConfig,
                          precision: Float = 1,
                          size: CGSize? = nil,
            ...
            -> Snapshotting
        {
            SimplySnapshotting.image(precision: precision).asyncPullback { viewController in
                snapshotView(config: size.map { .init(safeArea: config.safeArea, size: $0, traits: config.traits) } ?? config,
        ...
        {
            SimplySnapshotting.image(precision: precision).asyncPullback { viewController in
                snapshotView(config: .init(safeArea: .zero, size: size, traits: traits),
    ...
    public extension Snapshotting where Value == UIViewController, Format == String {
        ...
        static var hierarchy: Snapshotting {
            Snapshotting<String, String>.lines.pullback { viewController in
                let dispose = prepareView(config: .init(),
                                          drawHierarchyInKeyWindow: false,
                                          traits: .init(),
                                          view: viewController.view,
                                          viewController: viewController)
        ...
        ///   - size: A view size override.
        ///   - traits: A trait collection override.
        static func recursiveDescription(on config: ViewImageConfig = .init(),
                                         size: CGSize? = nil,
                                         traits: UITraitCollection = .init())
            -> Snapshotting<UIViewController, String>
        {
            SimplySnapshotting.lines.pullback { viewController in
                let dispose = prepareView(config: .init(safeArea: config.safeArea, size: size ?? config.size, traits: config.traits),
                                          drawHierarchyInKeyWindow: false,
                                          traits: traits,
                                          view: viewController.view,
                                          viewController: viewController)
