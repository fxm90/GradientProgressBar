    ...
    public extension Diffing where Value == NSImage {
        ...
        static let image = Diffing.image(precision: 1)

        /// A pixel-diffing strategy for NSImage that allows customizing how precise the matching must be.
        ///
        /// - Parameter precision: A value between 0 and 1, where 1 means the images must match 100% of their pixels.
        /// - Returns: A new diffing strategy.
        static func image(precision: Float) -> Diffing {
            .init(toData: { NSImagePNGRepresentation($0)! },
                  fromData: { NSImage(data: $0)! }) { old, new in
    ...
    private func diff(_ old: NSImage, _ new: NSImage) -> NSImage {
        ...
        let maxSize = CGSize(width: max(old.size.width, new.size.width),
