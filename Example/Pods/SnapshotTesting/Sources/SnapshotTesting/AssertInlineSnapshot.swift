...
public func _assertInlineSnapshot<Value>(matching value: @autoclosure () throws -> Value,
                                         ...
                                         line: UInt = #line)
{
    let failure = _verifyInlineSnapshot(matching: try value(),
                                        as: snapshotting,
                                        record: recording,
                                        timeout: timeout,
                                        with: reference,
...
public func _verifyInlineSnapshot<Value>(matching value: @autoclosure () throws -> Value,
                                         as snapshotting: Snapshotting<Value, String>,
                                         record recording: Bool = false,
                                         timeout: TimeInterval = 5,
                                         with reference: String,
...
internal struct Context {
    ...
    func setSourceCode(_ newSourceCode: String) -> Context {
        Context(sourceCode: newSourceCode,
                diffable: diffable,
                fileName: fileName,
                lineIndex: lineIndex)
    }
...
internal func writeInlineSnapshot(_ recordings: inout Recordings,
                                  _ context: Context) throws -> Context
{
    ...
    if let multiLineLiteralEndIndex = sourceCodeLines[offsetStartIndex...].firstIndex(where: { $0.hasClosingMultilineStringDelimiter() }) {
        ...
        // Update opening (#...)"""
        sourceCodeLines[functionLineIndex].replaceFirstOccurrence(of: extendedOpeningStringDelimitersPattern,
                                                                  with: multiLineStringLiteralTerminatorPre)

        // Update closing """(#...)
        sourceCodeLines[multiLineLiteralEndIndex].replaceFirstOccurrence(of: extendedClosingStringDelimitersPattern,
